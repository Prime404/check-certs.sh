...
if [ "$DATASOURCE" == "MYSQL" ] ; then
  ...
  domains="$(mysql -N -B -h "$MYSQL_HOST" -u "$MYSQL_USER" -p"$MYSQL_PASS" "$MYSQL_DB" -e "SELECT \`${MYSQL_COL}\` from \`${MYSQL_TABLE}\`;" | sort -u | sed 's/\*.//')"
elif [ "$DATASOURCE" == "LOCAL_TXT" ] && [ -f "$TXT_SOURCE" ] ; then
  domains="$(cat $TXT_SOURCE)"
fi
